...
import { availabilityLabels, safeSourceURL, showError, showTime, supportLabels } from './showPrepLabels';
...
export function ShowSupport({ evaluation: e }: { evaluation: ShowEvaluation }) {
  return <div className="show-support tabular-nums">
    <strong className={e.status === 'supported' ? 'text-[var(--success)]' : 'text-[var(--warning)]'}>{supportLabels[e.status] ?? 'Needs review'}</strong>
    ...
    <span>30d median <b>{e.compCount > 0 ? formatCents(e.medianCents) : 'Unknown'}</b> · {e.compCount} sales</span>
    <span>Latest sale {e.latestSaleDate || 'Unknown'}</span>
...
export default function ShowEvidenceDisclosure({ purchaseId, certNumber, evaluation, loading, error, actions, expanded, onExpandedChange }: {
  ...
  return <div className="show-disclosure">
    ...
    <div className="show-actions"><button type="button" className="show-link" aria-expanded={open} aria-controls={regionId}
