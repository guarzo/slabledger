...
it('reports one inline-price failure and preserves the original rejection through the full error body', async () => {
  let body!: ReadableStreamDefaultController<Uint8Array>;
  const fetch = vi.fn(async () => new Response(new ReadableStream({ start(controller) { body = controller; } }), { status: 400 }));
  vi.stubGlobal('fetch', fetch);
  const save = vi.spyOn(api, 'setReviewedPrice');
  const { result, qc, unmount } = setup(); let outcome!: Promise<unknown>; let settled = false;
  act(() => { outcome = result.current.handleInlinePriceSave('purchase-1', 30000).catch(error => error).finally(() => { settled = true; }); });
  await waitFor(() => expect(fetch).toHaveBeenCalledOnce()); expect(settled).toBe(false);
...
it('saves the original cents payload, notifies once and invalidates inventory after success', async () => {
  const fetch = vi.fn(async () => Response.json({ success: true, reviewedAt: '2026-09-15T12:00:00Z' })); vi.stubGlobal('fetch', fetch);
  const { result, qc, unmount } = setup();
  await act(async () => { await expect(result.current.handleInlinePriceSave('purchase-1', 30000)).resolves.toBeUndefined(); });
  expect(fetch).toHaveBeenCalledWith('/api/purchases/purchase-1/review-price', expect.objectContaining({ method: 'PATCH', body: JSON.stringify({ priceCents: 30000, source: 'manual' }) }));
  expect(screen.getAllByRole('button', { name: 'Close notification' })).toHaveLength(1);
  expect(screen.getByText('Price saved')).toBeInTheDocument(); expect(qc.getQueryState(queryKeys.portfolio.globalInventory)?.isInvalidated).toBe(true);
  unmount(); qc.clear();
});
it('keeps a self-reported resolve-flag failure non-rejecting with one notification', async () => {
  ...
  expect(screen.getAllByRole('button', { name: 'Close notification' })).toHaveLength(1);
  expect(screen.getByText('Flag rejected')).toBeInTheDocument(); unmount(); qc.clear();
});
