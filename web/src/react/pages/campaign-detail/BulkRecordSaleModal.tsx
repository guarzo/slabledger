  ...
  return (
    <Dialog.Root open={open} onOpenChange={(isOpen) => { if (!isOpen) handleClose(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed right-0 top-0 bottom-0 z-50 w-[min(560px,calc(100%-2rem))] bg-[var(--surface-1)] border-l border-[var(--surface-2)] p-6 shadow-2xl data-[state=open]:animate-[slideInFromRight_200ms_cubic-bezier(0.4,0,0.2,1)] overflow-y-auto"
        >
          <Dialog.Title className="text-lg font-semibold text-[var(--text)] mb-4">
            Record Sale ({pendingItems.length} cards)
          </Dialog.Title>
          <Dialog.Description className="sr-only">
            Enter sale details for {pendingItems.length} cards. Choose a pricing mode (percent of CL or flat dollar) and fill all rows at once, or edit individual sale prices below.
          </Dialog.Description>
