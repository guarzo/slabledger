...
describe('buildPriceSources', () => {
  it('includes Cost and CL first, then Market and Last Sold at end when mmCents absent', () => {
    ...
    expect(sources[2]).toMatchObject({ label: 'Market', source: 'market', priceCents: 2000 });
    expect(sources[3]).toMatchObject({ label: 'Last Sold', source: 'last_sold', priceCents: 1800 });
  });

  it('inserts MM between CL and Market when mmCents is present and positive', () => {
    const sources = buildPriceSources({
      clCents: 1000,
      marketCents: 2000,
      costCents: 500,
    ...
    expect(sources[3]).toMatchObject({ label: 'Market', source: 'market', priceCents: 2000 });
