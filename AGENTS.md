...
Project instructions for Pi and other coding agents. Go 1.26 / React 19 graded-card
resale ledger. Hexagonal (Clean) Architecture with PostgreSQL.

## Critical Rules

- Domain code (`internal/domain/`) must NEVER import adapter code (`internal/adapters/`). `make check` enforces this.
- All monetary values are stored as INTEGER cents internally. Convert to USD only where an API explicitly requires it; campaign API responses remain in cents.
