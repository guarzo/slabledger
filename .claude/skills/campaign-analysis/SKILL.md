...
1. **"What about liquidation?"** — pull `/api/portfolio/aging` and `/api/inventory` filtered to ≥30 days on eBay. Decisions look like: "Move 47 cards from Tier 2-3 inventory to LGS batch — expected $19K cash, P&L ~−$340 (R-017 LGS math)."
   - *Sell-side channel leak — investigated & falsified (2026-06-25, W-005).* A 4-stage in-person-vs-eBay analysis (`docs/private/sell-side-analysis.sh`, Postgres/External-excluded) found **no measurable channel leak**: the −$14.5K standard-campaign in-person loss is **forced liquidation of cards whose market fell after purchase**, not profitable cards mis-routed away from eBay. A naïve "segment median eBay net − in-person net" manufactures a fake ~$40–66 "leak" that is really grade-mixing + post-purchase drift (sell-side instance of R-025/R-027). **Don't re-chase this** without controlling for **same character + same grade + tight time window**; re-run the committed SQL if revisiting.
