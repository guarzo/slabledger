...
func TestDetectPostType_InsufficientCards_NewArrivals(t *testing.T) {
	repo := &mockSocialRepo{}
	// Return fewer than minCards recent purchases
	repo.getRecentPurchaseIDsFunc = func(_ context.Context, _ string) ([]string, error) {
		return []string{}, nil // empty — below minCards (default 1)
	}
	svc := NewService(repo).(*service)
	snapshots := []PurchaseSnapshot{}
	post, err := svc.detectPostType(context.Background(), PostTypeNewArrivals, snapshots)
	if post != nil {
...
func TestDetectPostType_InsufficientCards_AfterDedup(t *testing.T) {
	recent := time.Now().UTC().Format("2006-01-02")
	repo := &mockSocialRepo{}
	// Return 2 candidates for price movers
	...
	// All candidates already in existing posts
	...
	post, err := svc.detectPostType(context.Background(), PostTypePriceMovers, snapshots)
	if post != nil {
		t.Error("expected nil post when all filtered out")
	}
	if !IsInsufficientCards(err) {
		t.Errorf("expected ErrInsufficientCards, got %v", err)
	}
...
func TestRuleBasedGenerate_SkipsInsufficientCards(t *testing.T) {
	repo := &mockSocialRepo{}
	// All recent purchases empty → all 3 post types get InsufficientCards
	repo.getUnsoldPurchasesFunc = func(_ context.Context) ([]PurchaseSnapshot, error) {
