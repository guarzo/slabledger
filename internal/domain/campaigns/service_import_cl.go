...
func (s *service) RefreshCLValuesGlobal(ctx context.Context, rows []CLExportRow) (*GlobalCLRefreshResult, error) {
	...
	for i, row := range rows {
		...
		if err := s.repo.UpdatePurchaseCLValue(ctx, purchase.ID, newCLCents, newPop); err != nil {
			result.Failed++
		...
		if s.priceProv != nil && needsSnapshotRecovery(purchase) {
			if err := s.repo.UpdatePurchaseSnapshotStatus(ctx, purchase.ID, SnapshotStatusPending, 0); err != nil {
				if s.logger != nil {
...
func (s *service) ImportCLExportGlobal(ctx context.Context, rows []CLExportRow) (*GlobalImportResult, error) {
	...
	for i, row := range rows {
		...
		if existing != nil {
			...
			if err := s.repo.UpdatePurchaseCLValue(ctx, existing.ID, newCLCents, newPop); err != nil {
				result.Failed++
				result.Errors = append(result.Errors, ImportError{Row: rowNum, Error: err.Error()})
				result.Results = append(result.Results, GlobalImportItemResult{
			...
			if s.priceProv != nil && needsSnapshotRecovery(existing) {
				if err := s.repo.UpdatePurchaseSnapshotStatus(ctx, existing.ID, SnapshotStatusPending, 0); err != nil && s.logger != nil {
					s.logger.Warn(ctx, "failed to set pending snapshot status",
		...
		case "matched":
			...
			if created, err := s.repo.GetPurchaseByCertNumber(ctx, "PSA", row.SlabSerial); err == nil && created != nil {
