...
func (s *dhListingService) ListPurchases(ctx context.Context, certNumbers []string) DHListingResult {
	...
	for _, cn := range sortedCerts {
		...
		if err := s.lister.UpdateInventoryStatus(ctx, p.DHInventoryID, DHStatusListed); err != nil {
		...
		if err := s.lister.SyncChannels(ctx, p.DHInventoryID, defaultChannels); err != nil {
			...
			if revertErr := s.lister.UpdateInventoryStatus(ctx, p.DHInventoryID, DHStatusInStock); revertErr != nil {
...
func (s *dhListingService) inlineMatchAndPush(ctx context.Context, p *Purchase) int {
	if p.CertNumber == "" {
	...
	for _, r := range pushResp.Results {
		...
		if s.fieldsUpdater != nil {
			if err := s.fieldsUpdater.UpdatePurchaseDHFields(ctx, p.ID, DHFieldsUpdate{
				CardID:            dhCardID,
		...
		if s.pushStatusUpdater != nil {
			if err := s.pushStatusUpdater.UpdatePurchaseDHPushStatus(ctx, p.ID, DHPushStatusMatched); err != nil {
...
func (s *dhListingService) resolveInlineDHCardID(ctx context.Context, resp *DHCertResolution, p *Purchase) (int, bool) {
...
func (s *dhListingService) markInlineUnmatched(ctx context.Context, p *Purchase, dhStatus string) {
	if s.pushStatusUpdater != nil {
		if err := s.pushStatusUpdater.UpdatePurchaseDHPushStatus(ctx, p.ID, DHPushStatusUnmatched); err != nil {
