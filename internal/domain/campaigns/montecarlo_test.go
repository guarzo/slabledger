...
func TestRunSimulation_Deterministic(t *testing.T) {
	history := []PurchaseWithSale{
		{Purchase: Purchase{BuyCostCents: 8500, PSASourcingFeeCents: 300, GradeValue: 9, CLValueCents: 15000},
			Sale: &Sale{NetProfitCents: 2000, SaleFeeCents: 1200}},
		{Purchase: Purchase{BuyCostCents: 5000, PSASourcingFeeCents: 300, GradeValue: 8, CLValueCents: 8000},
			Sale: &Sale{NetProfitCents: 1000, SaleFeeCents: 800}},
		{Purchase: Purchase{BuyCostCents: 10000, PSASourcingFeeCents: 300, GradeValue: 10, CLValueCents: 20000},
			Sale: &Sale{NetProfitCents: 5000, SaleFeeCents: 2000}},
		{Purchase: Purchase{BuyCostCents: 7000, PSASourcingFeeCents: 300, GradeValue: 9.5, CLValueCents: 12000}},
		{Purchase: Purchase{BuyCostCents: 6000, PSASourcingFeeCents: 300, GradeValue: 8, CLValueCents: 10000},
			Sale: &Sale{NetProfitCents: -500, SaleFeeCents: 900}},
	}
...
func TestRunMonteCarloProjection_DecimalGrade(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		if i%2 == 0 {
			pw.Sale = &Sale{NetProfitCents: 1500, SaleFeeCents: 900}
		}
		history = append(history, pw)
	}

	campaign := &Campaign{BuyTermsCLPct: 0.60, GradeRange: "9.5-9.5"}
...
func TestRunMonteCarloProjection_InsufficientData(t *testing.T) {
	campaign := &Campaign{BuyTermsCLPct: 0.60, GradeRange: "8-10"}
	history := []PurchaseWithSale{
		{Purchase: Purchase{BuyCostCents: 5000, GradeValue: 9}},
	}
