...
func (s *service) processSnapshotsByStatus(ctx context.Context, status SnapshotStatus, limit int) (processed, skipped, failed int, err error) {
	...
	for _, p := range purchases {
		...
		case snapshotSkipped:
			...
			if err := s.purchases.UpdatePurchaseSnapshotStatus(ctx, p.ID, SnapshotStatusExhausted, p.SnapshotRetryCount); err != nil && s.logger != nil {
		...
		case snapshotProviderError, snapshotDBError:
			...
			if err := s.purchases.UpdatePurchaseSnapshotStatus(ctx, p.ID, newStatus, newCount); err != nil && s.logger != nil {
