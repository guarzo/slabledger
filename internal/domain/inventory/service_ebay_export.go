...
func (s *service) ListEbayExportItems(ctx context.Context, flaggedOnly bool) (*EbayExportListResponse, error) {
	...
	if flaggedOnly {
		purchases, err = s.purchases.ListEbayFlaggedPurchases(ctx)
	} else {
...
func (s *service) GenerateEbayCSV(ctx context.Context, items []EbayExportGenerateItem) ([]byte, error) {
	...
	purchases, err := s.purchases.GetPurchasesByIDs(ctx, purchaseIDs)
	if err != nil {
	...
	if err := s.purchases.ClearEbayExportFlags(ctx, purchaseIDs); err != nil {
