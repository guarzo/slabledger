...
func TestMaxToolRounds_Exceeded(t *testing.T) {
	...
	// OpDigest has a per-operation override of 4 rounds, so we need 5
	// responses to guarantee the loop hits the cap.
	llm := &mockLLMProvider{
		responses: []func(CompletionRequest, func(CompletionChunk)) error{
			chunkToolCall("tc-1", "list_campaigns", `{}`),
	...
	// OpDigest override = 4 rounds, so LLM should have been called 4 times.
	if llm.calls != 4 {
