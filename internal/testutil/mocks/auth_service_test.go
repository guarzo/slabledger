...
// newSvc builds an auth.Service double backed by a fresh in-memory repository.
// opts are forwarded to NewInMemoryAuthService; pass mocks.WithAuthLoginURLFn(...)
// for URL tests.
func newSvc(t *testing.T, opts ...mocks.InMemoryAuthServiceOption) (auth.Service, *mocks.InMemoryAuthRepository) {
	t.Helper()
	repo := mocks.NewInMemoryAuthRepository()
	svc, err := mocks.NewInMemoryAuthService(repo, opts...)
	if err != nil {
		t.Fatalf("mocks.NewInMemoryAuthService: %v", err)
	}
	return svc, repo
}
...
func TestNewInMemoryAuthService_NilRepo(t *testing.T) {
	_, err := mocks.NewInMemoryAuthService(nil)
	if err == nil {
...
func TestInMemoryAuthService_GetOrCreateUser(t *testing.T) {
	tests := []struct {
		name         string
...
func TestInMemoryAuthService_Session(t *testing.T) {
	tests := []struct {
		name string
		...
		{
			name: "created session uses the shared default expiry window",
			run: func(t *testing.T, svc auth.Service, _ *mocks.InMemoryAuthRepository) {
				ctx := context.Background()
				u := mustCreateUser(t, svc, "g-1b", "bea", "bea@example.com")
				...
				if diff := got - auth.DefaultSessionExpiry; diff > time.Second || diff < -time.Second {
					t.Errorf("session window = %v, want %v", got, auth.DefaultSessionExpiry)
				}
			},
		},
		{
...
func TestInMemoryAuthService_OAuthState(t *testing.T) {
	tests := []struct {
		name    string
		state   string
...
func TestInMemoryAuthService_EmailAllowlist(t *testing.T) {
	tests := []struct {
		name string
...
func TestInMemoryAuthService_GetUserByID(t *testing.T) {
	tests := []struct {
		name         string
...
func TestInMemoryAuthService_UpdateLastLogin(t *testing.T) {
	tests := []struct {
		name     string
		userID   int64
		seedUser bool
...
func TestInMemoryAuthService_AdminManagement(t *testing.T) {
	tests := []struct {
		name      string
		setAdmin  bool
		wantAdmin bool
...
func TestInMemoryAuthService_GetLoginURL(t *testing.T) {
	tests := []struct {
		name    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var opts []mocks.InMemoryAuthServiceOption
			if tt.fn != nil {
				opts = append(opts, mocks.WithAuthLoginURLFn(tt.fn))
			}
			svc, _ := newSvc(t, opts...)
...
func TestInMemoryAuthService_StoreTokens(t *testing.T) {
	svc, _ := newSvc(t)
	ctx := context.Background()
...
func TestInMemoryAuthService_NotImplemented(t *testing.T) {
	tests := []struct {
		name string
