...
func (s *DHOrdersPollScheduler) poll(ctx context.Context) {
	...
	s.logger.Info(ctx, "dh orders poll received orders",
		observability.Int("count", len(resp.Orders)),
		observability.String("since", since))

	// 4. Convert DH orders to OrdersExportRow + build certNumber->orderID map
	rows := make([]campaigns.OrdersExportRow, 0, len(resp.Orders))
	certToOrderID := make(map[string]string, len(resp.Orders))
	...
	// 5. Import — match certs to purchases
...
func parseDHSoldAt(soldAt string) string {
	...
	if err != nil {
		return soldAt // return as-is if parse fails
	}
