...
func TestEnsureProperties(t *testing.T) {
	t.Run("adds missing properties to object type", func(t *testing.T) {
		input := map[string]any{
			"type": "object",
		}
		result := ensureProperties(input)
	...
	t.Run("handles nil properties", func(t *testing.T) {
		...
		m := result.(map[string]any)
		if m["properties"] == nil {
			t.Error("expected nil properties to be replaced with empty map")
		}
	})

	t.Run("recurses into nested schemas", func(t *testing.T) {
		input := map[string]any{
			"type": "object",
			"properties": map[string]any{
		...
		if _, ok := nested["properties"]; !ok {
			t.Error("expected nested object to have properties added")
		}
	})

	t.Run("handles non-map input via JSON roundtrip", func(t *testing.T) {
		type testSchema struct {
			Type string `json:"type"`
		}
		input := testSchema{Type: "string"}
		result := ensureProperties(input)
		m, ok := result.(map[string]any)
		if !ok {
			t.Fatal("expected map[string]any from JSON roundtrip")
		}
		if m["type"] != "string" {
