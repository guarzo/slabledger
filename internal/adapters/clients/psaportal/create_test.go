...
func TestCreateCampaign_PostsBareFormDataAndDecodesID(t *testing.T) {
	...
	routes["/_app/remote/abc123/createCampaign"] = `{"type":"result","result":"[{\"campaignRequestId\":1,\"status\":2},\"uuid-new-1\",\"PAUSED\"]"}`
	...
	payloadStr := extractPayload(t, ff.captured["/_app/remote/abc123/createCampaign"])
...
func TestCreateCampaign_Failures(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			routes := bundleRoutes()
			routes["/_app/remote/abc123/createCampaign"] = tt.response
			ff := &fakeFetcher{
				routes:    routes,
				statusFor: map[string]int{"/_app/remote/abc123/createCampaign": tt.statusCode},
			}
