...
func (as *AnalyticsStore) GetDailySpend(ctx context.Context, campaignID string, days int) ([]inventory.DailySpend, error) {
	query := `
		...
		WHERE campaign_id = $1 AND purchase_date >= TO_CHAR(CURRENT_DATE - ($2 || ' days')::INTERVAL, 'YYYY-MM-DD')
...
func (as *AnalyticsStore) GetDaysToSellDistribution(ctx context.Context, campaignID string) ([]inventory.DaysToSellBucket, error) {
	query := `
		...
		INNER JOIN campaign_purchases p ON p.id = s.purchase_id
		WHERE p.campaign_id = $1
		GROUP BY label, min_val, max_val
...
func (as *AnalyticsStore) GetPerformanceByGrade(ctx context.Context, campaignID string) ([]inventory.GradePerformance, error) {
	query := `
		SELECT
			...
			AVG(CASE WHEN p.cl_value_cents > 0 THEN CAST(p.buy_cost_cents AS DOUBLE PRECISION) / p.cl_value_cents ELSE NULL END) AS avg_buy_pct_of_cl
		FROM campaign_purchases p
		LEFT JOIN campaign_sales s ON s.purchase_id = p.id
		WHERE p.campaign_id = $1
...
func (as *AnalyticsStore) GetAllPurchasesWithSales(ctx context.Context, opts ...inventory.PurchaseFilterOpt) ([]inventory.PurchaseWithSale, error) {
	...
	if f.SinceDate != "" {
		conditions = append(conditions, fmt.Sprintf("p.purchase_date >= $%d", len(args)+1))
...
// excludedPurchasesJoin joins campaign_sales -> campaign_purchases and excludes
// the external (Shopify-imported) campaign. Callers bind inventory.ExternalCampaignID
// at $1; any additional parameters must start at $2.
const excludedPurchasesJoin = `INNER JOIN campaign_purchases p ON p.id = s.purchase_id
		WHERE p.campaign_id <> $1`
