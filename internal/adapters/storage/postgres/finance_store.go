...
func (fs *FinanceStore) UpdateInvoice(ctx context.Context, inv *inventory.Invoice) error {
	result, err := fs.db.ExecContext(ctx,
		`UPDATE invoices SET total_cents = $1, paid_cents = $2, due_date = $3, paid_date = $4, status = $5, updated_at = $6 WHERE id = $7`,
...
func (fs *FinanceStore) GetCashflowConfig(ctx context.Context) (*inventory.CashflowConfig, error) {
	// credit_limit_cents is the legacy column name; the Go field is CapitalBudgetCents
	// (see migration note). Column name preserved for data continuity.
...
func (fs *FinanceStore) UpdateCashflowConfig(ctx context.Context, cfg *inventory.CashflowConfig) error {
	_, err := fs.db.ExecContext(ctx,
		`INSERT INTO cashflow_config (id, credit_limit_cents, cash_buffer_cents, updated_at) VALUES (1, $1, $2, $3)
		ON CONFLICT(id) DO UPDATE SET credit_limit_cents = $4, cash_buffer_cents = $5, updated_at = $6`,
...
func (fs *FinanceStore) GetCapitalRawData(ctx context.Context) (*inventory.CapitalRawData, error) {
	...
	err = fs.db.QueryRowContext(ctx,
		`SELECT
			COALESCE(SUM(CASE WHEN sale_date >= TO_CHAR(CURRENT_DATE - INTERVAL '30 days', 'YYYY-MM-DD') THEN sale_price_cents ELSE 0 END), 0),
			COALESCE(SUM(CASE WHEN sale_date >= TO_CHAR(CURRENT_DATE - INTERVAL '60 days', 'YYYY-MM-DD') AND sale_date < TO_CHAR(CURRENT_DATE - INTERVAL '30 days', 'YYYY-MM-DD') THEN sale_price_cents ELSE 0 END), 0)
		FROM campaign_sales`,
...
func (fs *FinanceStore) UpdateRevocationFlagStatus(ctx context.Context, id string, status string, sentAt *time.Time) error {
	query := `UPDATE revocation_flags SET status = $1, sent_at = $2 WHERE id = $3`
