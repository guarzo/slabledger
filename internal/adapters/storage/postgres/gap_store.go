...
func (s *GapStore) GetGapReport(ctx context.Context, since time.Time) (*scoring.GapReport, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT COUNT(DISTINCT entity_type || '|' || entity_id), COUNT(*) FROM scoring_data_gaps WHERE recorded_at >= $1`, since).
	...
	setRows, err := s.db.QueryContext(ctx,
		`SELECT set_name, COUNT(*) as cnt, STRING_AGG(DISTINCT factor_name, ',') as factors
		 FROM scoring_data_gaps
		 WHERE recorded_at >= $1 AND set_name != ''
		 GROUP BY set_name
		 ORDER BY cnt DESC
		 LIMIT 10`, since)
	if err != nil {
...
func (s *GapStore) PruneOldGaps(ctx context.Context, olderThan time.Time) (int64, error) {
	result, err := s.db.ExecContext(ctx,
		`DELETE FROM scoring_data_gaps WHERE recorded_at < $1`, olderThan)
	if err != nil {
		return 0, err
	}
