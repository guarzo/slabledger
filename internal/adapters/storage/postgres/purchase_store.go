...
// clause below. purchase_date is TEXT (YYYY-MM-DD, migration 000001) and is
// validated for non-emptiness only (validation.go:193,216), so it can hold
// arbitrary client text. It is therefore never CAST here.
//
	...
	return ps.execAndExpectRow(ctx, "update cl value",
		`UPDATE campaign_purchases SET
		 ...
		 cl_value_at_purchase_source = CASE
		   WHEN cl_value_at_purchase_cents = 0 AND $1::bigint > 0 AND `+clFreezeLateness+`
		   THEN 'cardladder' ELSE cl_value_at_purchase_source END,
