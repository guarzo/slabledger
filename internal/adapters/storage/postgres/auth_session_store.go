...
func (r *AuthRepository) GetSession(ctx context.Context, sessionID string) (*auth.Session, error) {
	...
	if tx, txErr := r.db.BeginTx(ctx, nil); txErr == nil {
		if _, err := tx.ExecContext(ctx, `UPDATE user_sessions SET id = $1 WHERE id = $2`, hashedID, sessionID); err != nil {
			//nolint:errcheck // best-effort migration rollback
			tx.Rollback()
		} else if _, err := tx.ExecContext(ctx, `UPDATE user_tokens SET session_id = $1 WHERE session_id = $2`, hashedID, sessionID); err != nil {
			//nolint:errcheck // best-effort migration rollback
			tx.Rollback()
...
func (r *AuthRepository) resolveSessionID(ctx context.Context, sessionID string) (string, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT 1 FROM user_sessions WHERE id = $1`, hashedID).Scan(&exists)
	if err == nil {
		return hashedID, nil
	}
	if !errors.Is(err, sql.ErrNoRows) {
	...
	err = r.db.QueryRowContext(ctx,
		`SELECT 1 FROM user_sessions WHERE id = $1`, sessionID).Scan(&exists)
	if err == nil {
...
func (r *AuthRepository) DeleteSession(ctx context.Context, sessionID string) error {
	...
	_, err = r.db.ExecContext(ctx, `DELETE FROM user_sessions WHERE id = $1`, dbID)
	if err != nil {
