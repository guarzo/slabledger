...
// PendingItemsRepository implements inventory.PendingItemRepository using Postgres.
type PendingItemsRepository struct {
	db *sql.DB
}

// NewPendingItemsRepository creates a new Postgres pending items repository.
...
func (r *PendingItemsRepository) SavePendingItems(ctx context.Context, items []inventory.PendingItem) error {
	...
	const updateSQL = `
		UPDATE psa_pending_items SET
			status = $1, candidates = $2, buy_cost_cents = $3,
			card_name = $4, set_name = $5, card_number = $6,
			grade = $7, purchase_date = $8, source = $9
		WHERE cert_number = $10 AND resolved_at IS NULL
	`
...
func (r *PendingItemsRepository) resolvePendingItem(ctx context.Context, id string, campaignID string) error {
	res, err := r.db.ExecContext(ctx, `
		...
		SET resolved_at = $1, resolved_campaign_id = $2
