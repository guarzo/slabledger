...
// PricingDiagnosticsRepository implements pricing.PricingDiagnosticsProvider using Postgres.
...
func (r *PricingDiagnosticsRepository) queryRecentFailures(ctx context.Context, diag *pricing.PricingDiagnostics) error {
	rows, err := r.db.QueryContext(ctx, `
		SELECT
			...
			CASE
				...
				WHEN error ILIKE '%not found%' OR error ILIKE '%no match%' THEN 'not_found'
	...
	for rows.Next() {
		...
		// api_calls.timestamp is TIMESTAMP; cast to text via MAX(...)::text or
		// let the driver coerce. parseTimestamp handles both RFC3339 and the
		// "2006-01-02 15:04:05" layout that Postgres emits.
		f.LastSeen = parseTimestamp(lastSeen)
