...
func (r *CampaignsRepository) GetCampaignPNL(ctx context.Context, campaignID string) (*campaigns.CampaignPNL, error) {
	query := `
		SELECT
		...
		WHERE p.campaign_id = ?
	`
	pnl := &campaigns.CampaignPNL{CampaignID: campaignID}
...
func (r *CampaignsRepository) GetPNLByChannel(ctx context.Context, campaignID string) ([]campaigns.ChannelPNL, error) {
	query := `
	...
	return scanRows(ctx, rows, func(rs *sql.Rows) (campaigns.ChannelPNL, error) {
		var ch campaigns.ChannelPNL
		err := rs.Scan(&ch.Channel, &ch.SaleCount, &ch.RevenueCents, &ch.FeesCents, &ch.NetProfitCents, &ch.AvgDaysToSell)
		return ch, err
	})
}

func (r *CampaignsRepository) GetDailySpend(ctx context.Context, campaignID string, days int) ([]campaigns.DailySpend, error) {
	query := `
	...
	return scanRows(ctx, rows, func(rs *sql.Rows) (campaigns.DailySpend, error) {
		var ds campaigns.DailySpend
		err := rs.Scan(&ds.Date, &ds.SpendCents, &ds.PurchaseCount)
		return ds, err
	})
}

func (r *CampaignsRepository) GetDaysToSellDistribution(ctx context.Context, campaignID string) ([]campaigns.DaysToSellBucket, error) {
	query := `
		SELECT
			CASE
	...
	return scanRows(ctx, rows, func(rs *sql.Rows) (campaigns.DaysToSellBucket, error) {
		var b campaigns.DaysToSellBucket
		err := rs.Scan(&b.Label, &b.Min, &b.Max, &b.Count)
		return b, err
	})
}

func (r *CampaignsRepository) GetPerformanceByGrade(ctx context.Context, campaignID string) ([]campaigns.GradePerformance, error) {
	query := `
		SELECT
	...
	return scanRows(ctx, rows, func(rs *sql.Rows) (campaigns.GradePerformance, error) {
		var g campaigns.GradePerformance
...
func (r *CampaignsRepository) GetPurchasesWithSales(ctx context.Context, campaignID string) ([]campaigns.PurchaseWithSale, error) {
	...
	return scanRows(ctx, rows, func(rs *sql.Rows) (campaigns.PurchaseWithSale, error) {
		return scanPurchaseWithSale(rs)
	})
}

func (r *CampaignsRepository) GetAllPurchasesWithSales(ctx context.Context, opts ...campaigns.PurchaseFilterOpt) ([]campaigns.PurchaseWithSale, error) {
	var f campaigns.PurchaseFilter
...
func (r *CampaignsRepository) GetPortfolioChannelVelocity(ctx context.Context) ([]campaigns.ChannelVelocity, error) {
	...
	return scanRows(ctx, rows, func(rs *sql.Rows) (campaigns.ChannelVelocity, error) {
		var cv campaigns.ChannelVelocity
		err := rs.Scan(&cv.Channel, &cv.SaleCount, &cv.AvgDaysToSell, &cv.RevenueCents)
		return cv, err
	})
}

func (r *CampaignsRepository) GetDailyCapitalTimeSeries(ctx context.Context) ([]campaigns.DailyCapitalPoint, error) {
	query := `
		WITH daily AS (
	...
	return scanRows(ctx, rows, func(rs *sql.Rows) (campaigns.DailyCapitalPoint, error) {
		var dp campaigns.DailyCapitalPoint
		err := rs.Scan(&dp.Date, &dp.CumulativeSpendCents, &dp.CumulativeRecoveryCents, &dp.OutstandingCents)
		return dp, err
	})
}

func (r *CampaignsRepository) GetGlobalPNLByChannel(ctx context.Context) ([]campaigns.ChannelPNL, error) {
	query := `
		SELECT s.sale_channel, COUNT(*) AS sale_count,
			SUM(s.sale_price_cents) AS revenue,
			SUM(s.sale_fee_cents) AS fees,
