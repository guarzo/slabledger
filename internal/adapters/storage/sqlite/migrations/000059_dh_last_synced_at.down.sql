-- Removes dh_last_synced_at from campaign_purchases.
-- SQLite does not support DROP COLUMN prior to 3.35.0; recreate without it.
...
CREATE TABLE campaign_purchases_new (
    ...
    cl_synced_at              TEXT DEFAULT '',
    psa_ship_date             TEXT NOT NULL DEFAULT '',
...
INSERT INTO campaign_purchases_new (
    ...
    created_at, updated_at, invoice_date, was_refunded,
    ...
    mm_value_updated_at, cl_synced_at, psa_ship_date, received_at
) SELECT
    id, campaign_id, card_name, card_number, set_name, cert_number, population,
    cl_value_cents, buy_cost_cents, psa_sourcing_fee_cents, purchase_date,
    last_sold_cents, lowest_list_cents, conservative_cents, median_cents,
