DROP TABLE IF EXISTS dh_suggestions;
DROP TABLE IF EXISTS market_intelligence;

-- Remove doubleholo provider from rate limits
DELETE FROM api_rate_limits WHERE provider = 'doubleholo';

-- Revert CHECK constraints to exclude 'doubleholo' (restore 000026 state)
...
INSERT INTO api_calls_new SELECT * FROM api_calls WHERE provider != 'doubleholo';
...
-- 2. price_refresh_queue: save and drop
CREATE TABLE price_refresh_queue_save AS SELECT * FROM price_refresh_queue WHERE source != 'doubleholo';
...
INSERT INTO api_rate_limits_new SELECT * FROM api_rate_limits WHERE provider != 'doubleholo';
DROP TABLE api_rate_limits;
ALTER TABLE api_rate_limits_new RENAME TO api_rate_limits;

-- 4. price_refresh_queue
