...
func TestCampaignsRepository_UpdatePurchaseGemRateID(t *testing.T) {
	repo := setupCampaignsRepo(t)
	ctx := context.Background()
	now := time.Now().Truncate(time.Second)

	// Create campaign + purchase
	require.NoError(t, repo.CreateCampaign(ctx, &campaigns.Campaign{
		ID: "c1", Name: "Test", Phase: campaigns.PhaseActive, CreatedAt: now, UpdatedAt: now,
	}))
	require.NoError(t, repo.CreatePurchase(ctx, &campaigns.Purchase{
		ID: "p1", CampaignID: "c1", CardName: "Charizard", CertNumber: "12345",
		PurchaseDate: "2026-01-01", CreatedAt: now, UpdatedAt: now,
	}))

	// Update gemRateID
	err := repo.UpdatePurchaseGemRateID(ctx, "p1", "abc123hash")
	require.NoError(t, err)

	got, err := repo.GetPurchase(ctx, "p1")
	require.NoError(t, err)
	assert.Equal(t, "abc123hash", got.GemRateID)

	// Not found
	err = repo.UpdatePurchaseGemRateID(ctx, "nonexistent", "abc")
	assert.ErrorIs(t, err, campaigns.ErrPurchaseNotFound)
}

func TestCampaignsRepository_UpdatePurchasePSASpecID(t *testing.T) {
	repo := setupCampaignsRepo(t)
	ctx := context.Background()
	now := time.Now().Truncate(time.Second)
	...
	err := repo.UpdatePurchasePSASpecID(ctx, "p1", 987654)
	require.NoError(t, err)

	got, err := repo.GetPurchase(ctx, "p1")
	require.NoError(t, err)
	assert.Equal(t, 987654, got.PSASpecID)

	err = repo.UpdatePurchasePSASpecID(ctx, "nonexistent", 1)
	assert.ErrorIs(t, err, campaigns.ErrPurchaseNotFound)
}
