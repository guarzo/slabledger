...
func setupSaleTestData(t *testing.T, db *DB, repo *testCampaignsRepository, campaignID, certNumber string) *inventory.Purchase {
	t.Helper()
...
func TestCreateSale(t *testing.T) {
	...
	t.Run("duplicate sale for same purchase", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, inventory.ErrDuplicateSale)
		assert.True(t, inventory.IsDuplicateSale(err))
	})
}
...
func TestGetSaleByPurchaseID(t *testing.T) {
	...
	t.Run("found", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, inventory.SaleChannelEbay, got.SaleChannel)
	...
	t.Run("not found", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, inventory.ErrSaleNotFound)
		assert.True(t, inventory.IsSaleNotFound(err))
	})
}
