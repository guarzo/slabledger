...
func TestGetCashflowConfig(t *testing.T) {
	...
	t.Run("returns stored config", func(t *testing.T) {
		...
		err := repo.UpdateCashflowConfig(ctx, &inventory.CashflowConfig{
	...
	t.Run("repeated updates overwrite existing row", func(t *testing.T) {
		...
		require.NoError(t, repo.UpdateCashflowConfig(ctx, &inventory.CashflowConfig{
...
func TestGetCapitalRawData_RecoveryVelocity(t *testing.T) {
	...
	t.Run("no sales returns raw zero recovery data", func(t *testing.T) {
		...
		assert.Equal(t, inventory.TrendStable, summary.RecoveryTrend)
	})
	...
	t.Run("recent sales compute recovery rate", func(t *testing.T) {
		...
		s := &inventory.Sale{
			ID: "sell-s1", PurchaseID: "sell-p0", SaleChannel: inventory.SaleChannelEbay,
		...
		assert.Equal(t, inventory.TrendImproving, summary.RecoveryTrend,
	...
	t.Run("raw data has both 30d and prior 30d recovery", func(t *testing.T) {
		...
		s1 := &inventory.Sale{
			ID: "trend-s1", PurchaseID: "trend-p1", SaleChannel: inventory.SaleChannelEbay,
		...
		s2 := &inventory.Sale{
			ID: "trend-s2", PurchaseID: "trend-p2", SaleChannel: inventory.SaleChannelEbay,
	...
	t.Run("declining recovery data from repo", func(t *testing.T) {
		...
		recentDate := time.Now().AddDate(0, 0, -5).Format("2006-01-02")
		s1 := &inventory.Sale{
			ID: "dec-s1", PurchaseID: "dec-p0", SaleChannel: inventory.SaleChannelEbay,
		...
		priorDate := time.Now().AddDate(0, 0, -45).Format("2006-01-02")
		s2 := &inventory.Sale{
			ID: "dec-s2", PurchaseID: "dec-p1", SaleChannel: inventory.SaleChannelEbay,
			SalePriceCents: 20000, SaleFeeCents: 2470, SaleDate: priorDate,
			DaysToSell: 5, NetProfitCents: 0, CreatedAt: now, UpdatedAt: now,
		}
		require.NoError(t, repo.CreateSale(ctx, s2))
		...
		// Verify derived fields via domain logic
		summary := inventory.ComputeCapitalSummary(raw)
		assert.Equal(t, inventory.TrendDeclining, summary.RecoveryTrend)
	})
	...
	t.Run("warning-level raw data from repo", func(t *testing.T) {
		...
		recentDate := time.Now().AddDate(0, 0, -5).Format("2006-01-02")
		s := &inventory.Sale{
			ID: "warn-s1", PurchaseID: "warn-p1", SaleChannel: inventory.SaleChannelEbay,
		...
		assert.Equal(t, inventory.AlertWarning, summary.AlertLevel)
	})
	...
	t.Run("critical-level raw data from repo", func(t *testing.T) {
		...
		recentDate := time.Now().AddDate(0, 0, -5).Format("2006-01-02")
		s := &inventory.Sale{
			ID: "alert-s1", PurchaseID: "alert-p2", SaleChannel: inventory.SaleChannelEbay,
			SalePriceCents: 100000, SaleFeeCents: 12350, SaleDate: recentDate,
			DaysToSell: 5, NetProfitCents: 0, CreatedAt: now, UpdatedAt: now,
		}
		require.NoError(t, repo.CreateSale(ctx, s))
		...
		assert.Equal(t, inventory.AlertCritical, summary.AlertLevel)
	})
...
func TestGetCapitalRawData_EmptyState(t *testing.T) {
	...
	assert.Equal(t, 99.0, summary.WeeksToCover)
	assert.Equal(t, inventory.TrendStable, summary.RecoveryTrend)
	assert.Equal(t, inventory.AlertOK, summary.AlertLevel)
}
...
func TestGetPendingReceiptByInvoiceDate(t *testing.T) {
	...
	tests := []struct {
		name        string
		setup       func(t *testing.T, db *DB, repo *testCampaignsRepository)
		inputDates  []string
...
func TestGetCapitalRawData_OutstandingAndPayments(t *testing.T) {
	...
	// Create campaign
	c := &inventory.Campaign{ID: "camp-credit", Name: "Credit Test", Phase: inventory.PhaseActive, PSASourcingFeeCents: 300, CreatedAt: now, UpdatedAt: now}
