...
**Goal:** Create the `/improve` skill — a holistic codebase review tool that runs quantitative analysis, performs qualitative code reading, and produces a force-ranked top-10 list of improvements with interactive follow-up.

**Architecture:** Single SKILL.md file in `.claude/skills/improve/` following established skill conventions (YAML frontmatter + step-based markdown). The skill instructs Claude through a two-phase process: run all available tooling first (Phase 1), then do targeted qualitative analysis informed by that data (Phase 2). Output is a top-10 prioritized list followed by interactive discussion.

**Tech Stack:** Claude Code skill (markdown), Bash commands (make, go, npm), Read/Grep/Glob for code analysis.
...
Create `.claude/skills/improve/SKILL.md` with the following complete content:
...
Run:
```bash
head -5 .claude/skills/improve/SKILL.md
```

Expected: the YAML frontmatter with `name: improve` and `description`.

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/improve/SKILL.md
git commit -m "feat: add /improve skill for holistic codebase review

Two-phase approach: quantitative sweep (make check, go test, lint,
coverage) followed by qualitative analysis across 8 categories.
Produces a force-ranked top-10 list with interactive follow-up
and light tracking via memory system."
```
...
The skill should be discoverable. Run `/improve` to test that the skill is recognized and starts executing. Let it run through Phase 1 at minimum to verify the commands work.

- [ ] **Step 2: Verify argument parsing**

Test with an invalid package name to confirm the validation logic works:
```
/improve nonexistent
```

Expected: the skill should list available packages and ask the user to pick one.

- [ ] **Step 3: Fix any issues found during smoke testing**

If the skill fails to parse, commands don't run, or output format is wrong, fix the SKILL.md and commit.
