...
- [ ] Keep hooks and state owner mounted across the view switch **and background inventory refetch failures**. `GlobalInventoryPage` currently returns an error screen on any `isError`, unmounting `InventoryTab` despite retained query data. Reserve the blocking error screen for an initial load with no successful data; with retained data, keep inventory mounted and show an inline read-error/retry notice. Distinguish an empty successful inventory from no successful response. Add a full-page regression with dirty drafts on two cards and bulk selection, a successful reviewed-price PATCH, then a failed **inventory GET**: focus, selection, and both drafts survive; retry issues only GET/read operations, never another price PATCH. Compose the new workspace below shared search/scope controls; normal inventory retains its established filters/default order/sales/matching/bulk behavior. Keep review-specific filter/order out of normal inventory sorting. Replace the old support control/type coupling rather than leaving a hidden second filter active.
...
- [ ] Explicitly compose the DH side effects in the new fixture. The existing `readinessRouter` only supplies `CampaignsService`, so its default handler has nil DH collaborators and cannot prove sync/list execution. Construct a `CampaignsHandler` with `handlers.WithDHPriceSyncer` and `handlers.WithDHListingService`, and pass that handler through `httpserver.RouterConfig.CampaignsHandler`. Use the real local persistence/composition with controlled DH boundaries, seed both an already-listed card and an eligible unlisted card, and record calls/results. Retain the configured handler and call `WaitBackground()` after relevant HTTP activity and before final state assertions or closing the database. A zero-call result must fail the positive side-effect checks.
- [ ] In a separately marked phase, explicitly save 240000 for the declining-market fixture through the actual reviewed-price route. Assert the intended reviewed-price persistence and refreshed assessment, plus **nonzero expected** controlled DH sync/list calls and exact permitted persisted changes, including eligible unlisted-to-listed behavior. Verify already-listed and ineligible cases against the existing service contract rather than assuming every save lists a card. Attribute these intentional writes separately from immutable preview/navigation proof; no misleading whole-flow zero-write claim.
...
Claude Opus 5 reviewed this plan at `c6b52195` and returned REVISE with two concerns. The operator accepted both and authorized subagent-driven execution: Task 5 now explicitly retains the inventory owner through a failed background inventory GET and tests read-only recovery; Task 6 now explicitly injects DH sync/list dependencies, seeds an eligible unlisted card, requires positive side-effect evidence, and joins handler background work before assertions/cleanup.
