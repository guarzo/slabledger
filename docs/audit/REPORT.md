...
| Findings filed | 63 |
| Confirmed | 48 |
| Confirmed, severity lowered | 11 |
| Refuted | 4 |
| **Confirmation rate** | **59/63 = 93%** |
| Ticketable | 54 |
| Fix units (tickets) | 35 |
...
- Ticketable findings: **54**
- Findings rolled into fix units: **55**
- ✅ Every ticketable finding appears in exactly one fix unit.
- Included by controller adjudication despite `ticketable: false`: DEADGO-004 — see the adjudication note on each unit.
...
**Proposed fix:** Add RLS (ENABLE ROW LEVEL SECURITY + service-role-bypass policy, matching 000003's pattern) for psa_campaign_push_queue in a new migration. Treat this one ahead of the others in DBSCHEMA-003 given the write-integrity consequence (forged approval rows reaching the live PSA portal).
...
**Proposed fix:** Add a new migration enabling RLS and a service-role-bypass policy for psa_portal_token, following the exact pattern used in 000003 (ALTER TABLE ... ENABLE ROW LEVEL SECURITY; CREATE POLICY "service role bypass" ON ... USING (true) WITH CHECK (true);). Consider also auditing whether this table needs to exist in the public Supabase project at all, given it holds token material even if encrypted.
...
**Proposed fix:** Add a single new migration enabling RLS + service-role-bypass policy (000003's pattern) for all five tables at once, alongside psa_portal_token (DBSCHEMA-001) and psa_campaign_push_queue (DBSCHEMA-002) if the team prefers one consolidated remediation migration instead of three.
...
**Proposed fix:** Two logically distinct removals, since the six methods split into a coherent token-lifecycle group and one unrelated method: (1) remove GetTokens, GetTokensByUserID, UpdateTokens, DeleteTokens, DeleteAllUserTokens from the auth.Repository interface, postgres.AuthRepository (auth_token_store.go), and both mocks — the per-session multi-device token read/update/delete path is entirely unused; only StoreTokens (create) is live. (2) separately remove CleanupExpiredOAuthStates from the interface, implementation (auth_oauth_state_store.go), and both mocks, or wire it into the existing session-cleanup scheduler (internal/adapters/scheduler/session_cleanup.go) if unbounded growth of the oauth_states table is an operational concern — that scheduler currently only calls CleanupExpiredSessions, not this method. Keep the auth.Repository interface itself; only these six methods and their implementations are in scope.
...
**Severity:** high · **Effort:** S · **Rolls up:** NB-001
...
**Proposed fix:** Derive SUB_PACKAGES from the filesystem (every directory under internal/domain/ that imports internal/domain/inventory) rather than hardcoding six names, so the rule cannot silently stop covering new packages. Then decide explicitly whether dhpricing -> dhlisting is an intended exception (and record it as one) or a violation to be broken by moving the shared types into inventory or a leaf package.
...
**Severity:** medium · **Effort:** S · **Rolls up:** DUP-003

> **Controller note.** A live defect, not a style issue: the local normalizer diverges from cardutil's shared version, causing silent disambiguation misses.

#### DUP-003 — DH cert-disambiguation's local card-number normalizer doesn't strip denominators, unlike cardutil's shared version, causing silent disambiguation misses
...
**Proposed fix:** Replace the body of internal/adapters/clients/dh/disambiguate.go's normalizeCardNumber with a call to cardutil.NormalizeCardNumber (or otherwise delegate to it), so denominator-bearing purchase card numbers (e.g. from ParsePSAListingTitle) can still be matched against bare-integer DH candidate numbers during ambiguous-cert resolution.
...
> **Controller note.** The domain fields designed to carry this rule (DailySpend.CapCents, FillRatePct) are never populated by the only repository, so the handler recomputes it — substituting the campaign's CURRENT cap for every historical day. The same rule is implemented a second time in the tuning rule.

#### ARCH-001 — Fill-rate business rule lives in an HTTP handler; the domain fields designed to carry it are never populated by the only repository
...
**Severity:** low · **Effort:** S · **Rolls up:** FE-007

#### FE-007 — Sale.forcedLiquidation is sent unconditionally by the Go API but has no field in the TS Sale type

*Lens:* `frontend-health` · *Confidence:* `strong` · *Verdict:* `confirmed_lower_severity` · *Severity:* low

**Subject:** `{'kind': 'type', 'identity': 'web/src/types/campaigns/core.ts Sale interface — missing field forcedLiquidation'}`

**Verifier correction:** The stated severity of 'medium' overstates the runtime consequence: a field present in the JSON response but absent from the TS interface is dropped by nothing — it simply isn't typed, so no runtime error, crash, or data loss occurs. TypeScript's structural typing means extra wire fields are inert until something tries to read them; this is a type-completeness/documentation gap, not a defect with an active symptom. The real consequence is that any future code wanting to read forcedLiquidation has no compiler-checked field to use, and a reviewer scanning the Sale type would not know the field exists.
...
**Proposed fix:** Add `forcedLiquidation: boolean;` to the Sale interface in web/src/types/campaigns/core.ts to match the Go contract. If the UI is meant to surface this (e.g. a badge distinguishing forced-liquidation sales in sale history/analytics), a follow-up UI task is separate from this type-sync fix.
...
**Severity:** medium · **Effort:** S · **Rolls up:** DEADGO-002

#### DEADGO-002 — AI image-generation cluster (ai.ImageGenerator + azureai.ImageClient) is fully defined but never constructed or wired anywhere

*Lens:* `dead-code-go` · *Confidence:* `mechanical` · *Verdict:* `confirmed` · *Severity:* medium
...
**Proposed fix:** Delete internal/adapters/clients/azureai/image_client.go and internal/domain/ai/image.go, unless there is a near-term product plan to wire AI image generation (advisor/campaign card imagery), in which case leave a tracking issue instead of deleting.
...
**Severity:** low · **Effort:** S · **Rolls up:** DEADGO-003

> **Controller note.** WithMaxTokens and WithTemperature are never called, so the fields they set are permanently stuck at their defaults. Confirm that freezing those defaults is intended before deleting rather than wiring.

#### DEADGO-003 — advisor.WithMaxTokens and advisor.WithTemperature functional options are never invoked; the fields they set are permanently stuck at their defaults
...
**Severity:** medium · **Effort:** M · **Rolls up:** DCT-014

> **Controller note.** ADJUDICATED — CONSTRAINED FIX. The finding's diagnosis is right: Config.Cache.Path is bound to -cache, syntax-validated, and has its directory created at startup, yet its value never reaches any cache constructor. But its proposed_fix offers 'remove the field, its CLI flag, and its validation/directory-creation code', and THAT OPTION BREAKS PRODUCTION: Dockerfile.harvest:39 passes --cache to the deployed entrypoint, and flag.ContinueOnError at loader.go:240 turns an unknown flag into log.Fatalf. Acceptance criteria MUST require that any removal edits Dockerfile.harvest:39 in the same commit, and MUST state that removing the flag registration alone fails the deployed container at startup with `flag provided but not defined: -cache`. Do NOT merge this with DEADGO-001 (refuted) — they share one wrong premise, and clustering them would launder the error rather than cancel it.

#### DCT-014 — Config.Cache.Path is validated and directory-created at startup but its value is never passed to any cache backend — the field is fully wired plumbing to nowhere
...
**Proposed fix:** Either wire cfg.Cache.Path into the actual cache backend construction path in cmd/slabledger (so the -cache flag/CACHE_PATH env var does something), or remove the field, its CLI flag, and its validation/directory-creation code if the file-based cache is meant to be configured a different way.
...
**Proposed fix:** Add a new migration dropping marketmovers_config (DROP TABLE IF EXISTS marketmovers_config;), mirroring how 000021 dropped its siblings. Also remove its entry from docs/SCHEMA.md (see DBSCHEMA-006).
...
**Severity:** medium · **Effort:** M · **Rolls up:** FE-002, FE-003

> **Controller note.** Same component trees, one PR. FE-003's verifier independently checked the factory-registration and test-only-usage escapes that the finding never claimed.

#### FE-002 — Seven React components in campaign-detail/inventory and advisor trees have no import site anywhere in web/src

*Lens:* `frontend-health` · *Confidence:* `strong` · *Verdict:* `confirmed` · *Severity:* medium
...
**Proposed fix:** Delete the 7 component files (and any co-located .module.css / test file that exists only for them) after confirming with git log / PR history that they are leftovers from a removed or superseded UI flow, not a component mid-integration.
...
**Proposed fix:** Remove the 11 unused hooks, or verify with the feature owner whether they are staged for an in-progress UI (campaign P&L view, portfolio velocity/insights widgets, bulk-sale/purchase-edit/DH-unmatch actions) that has not yet been wired to a page.
...
**Severity:** medium · **Effort:** M · **Rolls up:** FE-001, FE-004, FE-006

> **Controller note.** The five pricing.ts interfaces describe a per-grade eBay/estimate pricing API that no longer exists — they are the type-level residue of the removed pricing sources.
...
**Proposed fix:** Remove the 5 unused methods (and their interface declarations) from campaigns.ts, campaignAnalytics.ts and campaignImports.ts, or confirm with the backend owner whether the underlying endpoints (DELETE /campaigns/:id, campaign suggestions, external bulk import, revocation-flag listing, PSA sheet sync) are still reachable from any UI path before deleting.
...
**Proposed fix:** Remove the 10 dead exports. Downgrade any that are demonstrably reserved for a near-term feature (e.g. saleChannelColors for a planned channel-color legend) to a code comment rather than a live export.
...
**Severity:** medium · **Effort:** S · **Rolls up:** FE-005

> **Controller note.** Mounted app-wide but useUserPreferences() is never called, so the whole context has no consumer. Decide removal vs. wiring before writing code — the provider being mounted suggests intent that was never finished.

#### FE-005 — UserPreferencesProvider is mounted app-wide but its own useUserPreferences() hook is never called — the whole context has no consumer
...
**Proposed fix:** Either wire a real consumer (the JSDoc at line 152 documents an intended addRecentPriceCheck/preferences usage that was apparently never finished) or remove the entire feature (Provider, hook, and its 4 backing types) if it was superseded by something else. Left as-is, the Provider still runs its state/localStorage-sync logic on every render for a value nothing reads.
...
*Lens:* `docs-config-tests` · *Confidence:* `strong` · *Verdict:* `confirmed` · *Severity:* high
...
**Proposed fix:** Regenerate the domain package tree in CLAUDE.md from `go list ./internal/domain/...`, removing cards/ and favorites/, and adding demand/, dhevents/, dhpricing/, liquidation/, psacampaign/ with one-line descriptions each.
...
**Proposed fix:** Update the adapters/clients/ tree in CLAUDE.md: drop pricelookup/ and tcgdex/, add cardladder/, dh/, dhlisting/, psa/, psaportal/ with one-line descriptions, and correct the PriceLookup adapter's real location (internal/domain/pricing/lookup/) elsewhere in the doc if it's referenced.
...
**Proposed fix:** Add a CardLadder group to the Key groups list (CARDLADDER_REFRESH_ENABLED, CARDLADDER_REFRESH_HOUR).
...
**Proposed fix:** Correct the sentence to name the seven interfaces that actually exist (drop SnapshotRepository), fix the count to 7 (or explicitly mention PendingItemRepository as an eighth, optional repository if that's the intended scope), and stop describing PurchaseRepository as 'focused' — either split it (see the size-and-complexity lens's related finding on PurchaseRepository's breadth) or describe it accurately as the exception.
...
*Lens:* `naming-and-boundaries` · *Confidence:* `suspected` · *Verdict:* `confirmed` · *Severity:* low
...
**Proposed fix:** Owned by the docs-config-tests lens. Correct the CLAUDE.md roster to the eight interfaces that exist.
...
**Proposed fix:** Add a `### table_name` section to docs/SCHEMA.md for each of card_price_trajectory, dh_card_tombstones, dh_comp_cache, dh_state_events, psa_portal_snapshot, psa_portal_token, and scheduler_run_stats, following the existing column/index/FK table format used throughout the doc.
...
**Proposed fix:** Delete the `### advisor_cache`, `### sell_sheet_items`, and `### mm_card_mappings` sections from docs/SCHEMA.md, or replace each with a one-line 'Dropped in migration NNNNN' note consistent with how other removed tables (e.g. psa_exchange_policy, cardhedger-era tables) are already handled elsewhere in the doc, if such a convention exists.
...
**Proposed fix:** Change both 'Added: migration 000017' citations in docs/SCHEMA.md to '000018' for psa_campaign_snapshot and psa_campaign_push_queue.
...
**Proposed fix:** Fix the migration number cited for price_history's removal (or remove the citation if the real migration number is unknown at this revision), and delete or clearly mark as historical the advisor_cache and marketmovers table/column sections.
...
**Severity:** medium · **Effort:** L · **Rolls up:** DCT-012

> **Controller note.** Documents removed marketmovers/advisor/sell-sheet endpoints and is missing at least 13 live ones.

#### DCT-012 — docs/API.md documents removed marketmovers/advisor/sell-sheet endpoints and is missing at least 13 live endpoints
...
**Proposed fix:** Remove the marketmovers, extra advisor, and extra sell-sheet endpoint sections from docs/API.md; add sections for the 13+ live routes identified. A field-level pass (request/response shapes) was explicitly out of scope for this sweep and would likely surface more drift.
...
**Severity:** medium · **Effort:** M · **Rolls up:** DCT-006, DCT-007, DCT-008, DCT-009, DCT-010, DCT-020

> **Controller note.** internal/README.md, docs/ARCHITECTURE.md, docs/LLM_USAGE.md, docs/DH_INVENTORY.md, docs/USER_GUIDE.md and one package doc comment all still describe a SQLite storage layer, a removed 'picks' package and its 3-stage AI pipeline, a Favorites feature that does not exist in the product, and a pipeline function that exists nowhere in the repo.
...
**Proposed fix:** Replace all storage/sqlite/ references with storage/postgres/, and regenerate the Large File Awareness table from current `wc -l` output, adding portfolio/service.go and cardladder_refresh.go.
...
**Proposed fix:** Replace the SQLite stack line, diagram box, and tree entry with Postgres; remove the picks/ package section; remove or clearly mark the Favorites/cards sections as removed.
...
**Proposed fix:** Remove the Picks pipeline section from docs/LLM_USAGE.md, or replace it with a description of whatever AI-driven feature (if any) superseded it.
...
**Proposed fix:** Update both file paths in the table to internal/adapters/storage/postgres/.
...
**Proposed fix:** Remove the Favorites sections from docs/USER_GUIDE.md, or restore the feature if it is intended to exist and was accidentally dropped (verify with product owner before assuming either direction).
...
**Proposed fix:** Either rename the doc comment's last pipeline step to the function that actually performs category resolution today, or remove the step if no equivalent exists (verify which by reading the current normalization call chain before editing).
...
**Severity:** low · **Effort:** S · **Rolls up:** DCT-013, DCT-015

> **Controller note.** Two declared vars are read by zero Go code; four more knobs are undocumented or mis-documented. Report variable NAMES only — never values.
...
**Proposed fix:** Remove BASE_URL and MEDIA_DIR from .env.example, or wire them into config.Config/loader.go if they were meant to configure something (verify intent before deleting, since env var presence alone doesn't rule out a planned-but-unimplemented feature).
...
*Lens:* `docs-config-tests` · *Confidence:* `suspected` · *Verdict:* `confirmed` · *Severity:* low
...
**Proposed fix:** Add ADVISOR_MAX_TOOL_ROUNDS as a live (uncommented) entry in .env.example, and uncomment CL_SEARCH_URL/SHUTDOWN_TIMEOUT_SECONDS/AZURE_AI_TIMEOUT or add a comment noting they have working defaults and don't need to be set.
...
**Severity:** medium · **Effort:** M · **Rolls up:** DCT-016

> **Controller note.** The precondition the TODO waits on was satisfied in the same commit that added the TODO, four months ago.

#### DCT-016 — A 4-month-stale TODO blocks re-enabling scheduler tests for a precondition that was satisfied in the same commit that added the TODO
...
**Proposed fix:** Write direct tests for refreshBatch covering: candidate-fetch error handling and consecutive-failure counting, empty-candidates no-op, provider-blocked skip, and hourly-budget-exhausted skip — then delete the stale TODO comment.
...
**Severity:** medium · **Effort:** M · **Rolls up:** DCT-017

> **Controller note.** PSA import batch translation, inventory sold-transitions, and PSA key-rotation delegation currently have zero tests.

#### DCT-017 — The dhlisting adapter package — PSA import batch translation, inventory sold-transitions, and PSA key-rotation delegation — has zero test coverage
...
**Proposed fix:** Add table-driven tests for PSAImporterAdapter.PSAImport (success, batch-rejected-with-reason, batch-rejected-without-reason) and InventoryAdapter's status-transition and rotation-delegation paths, following the internal/testutil/mocks Fn-field pattern used elsewhere in the adapters layer.
...
**Severity:** medium · **Effort:** S · **Rolls up:** NB-005, NB-006

> **Controller note.** Pure renames plus content moves, one PR. analytics_types.go and types_analytics.go coexist under two opposite conventions, and the latter mostly holds capital/finance and portfolio-health types. service_advanced.go names an adjective, not a concept.
...
*Lens:* `naming-and-boundaries` · *Confidence:* `strong` · *Verdict:* `confirmed` · *Severity:* medium
...
**Proposed fix:** Pick one convention (the package majority is `<subject>_types.go`) and rename consistently. Then split types_analytics.go by its actual contents: capital_types.go (CapitalRawData, CapitalSummary, InvoiceSellThrough, ComputeCapitalSummary), health_types.go (PortfolioHealth, CampaignHealth, ChannelVelocity, AlertLevel, RecoveryTrend) and purchase_filter.go (PurchaseFilter, PurchaseFilterOpt, With*). These are file renames within one package — no import path changes and no behavior change.
...
**Proposed fix:** Delete the file by relocating its two methods: LookupCert alongside the other cert-entry code, QuickAddPurchase alongside the purchase CRUD in service_crud.go. If service_cert_entry.go is already at its size budget, create service_cert_lookup.go rather than reintroducing an adjective-named file.
...
**Severity:** low · **Effort:** S · **Rolls up:** NB-008

#### NB-008 — Invoice projection — an unambiguously financial concept — sits in `inventory` with `internal/domain/finance` as its only consumer

*Lens:* `naming-and-boundaries` · *Confidence:* `strong` · *Verdict:* `confirmed` · *Severity:* low

**Subject:** `{'kind': 'symbol', 'identity': 'internal/domain/inventory/invoice_projection.go'}`

**Verifier correction:** Evidence claim 2 ('InvoiceProjection itself has no consumer outside the file that declares it') is literally true but reads as stronger than it is: finance does consume the type, via `projection := inventory.ComputeInvoiceProjection(...)` at service.go:49, where type inference means the identifier never appears in the source text. The grep measures textual absence, not absence of use. The conclusion is unaffected — the consumer is finance either way, which is the package the finding proposes moving to.
...
**Proposed fix:** Move invoice_projection.go into internal/domain/finance/. finance already imports inventory, so no new sibling edge is created and the flat-sibling rule is unaffected. Bundle this with NB-002 as a single 'reunite each sibling with its own computation' change rather than shipping it alone.
...
**Severity:** high · **Effort:** M · **Rolls up:** NB-003

> **Controller note.** 919 LOC with zero inbound references from the rest of `inventory` and exactly one consumer package — the cleanest seam the audit found.

#### NB-003 — The 919-LOC campaign-suggestions cluster has zero inbound references from the rest of `inventory` and exactly one consumer package — a clean, isolated seam
...
**Severity:** medium · **Effort:** M · **Rolls up:** SIZE-002

#### SIZE-002 — ListPurchases is a 286-line function fusing batch aggregation with a large per-purchase state machine
...
> **Controller note.** ~20 independent, self-contained blocks that could each be its own function.

#### SIZE-003 — BuildGroup is a 312-line function built from ~20 independent, self-contained scheduler-construction blocks that could each be its own function
...
**Severity:** low · **Effort:** M · **Rolls up:** SIZE-004

> **Controller note.** 1,328 lines / 106 methods, invisible to check-file-size.sh because testutil is excluded. The exclusion is arguably correct; the file size is not.

#### SIZE-004 — inmemory_campaign_store.go is 1,328 lines / 106 methods, invisible to check-file-size.sh because testutil is excluded
...
**Severity:** low · **Effort:** L · **Rolls up:** ARCH-002

> **Controller note.** Opaque vendor JSON blobs pass through the domain; writer and reader share no type, so the compiler enforces nothing across the boundary.

#### ARCH-002 — The demand repository contract passes opaque vendor JSON blobs through the domain; writer and reader share no Go type, so the compiler enforces nothing across the seam
...
**Severity:** low · **Effort:** S · **Rolls up:** ARCH-003

> **Controller note.** Duplicated as an unexported constant in two adapter packages that are forbidden from importing each other — the duplication is a direct consequence of the architecture rule, so the fix belongs in the domain.

#### ARCH-003 — The DH tombstone policy has no home in the domain: the threshold is duplicated as an unexported constant in two adapter packages that are forbidden from importing each other
...
**Proposed fix:** Add an exported constant (or an IsTombstoned(attempts int) bool predicate) to internal/domain/pricing next to DHCardTombstoneRepo, and have both adapters reference it. Replace the 'attempts >= 3' prose in the interface doc with a pointer to the constant.
...
**Severity:** low · **Effort:** S · **Rolls up:** DUP-001, DUP-002

> **Controller note.** Both are 'call the existing shared function instead'. Note the verifier re-grounded DUP-002: it is a pure maintainability concern, NOT a demonstrated live defect — the divergence cannot be exercised by any valid grader title. Do not let the ticket imply a live bug.
...
**Proposed fix:** Delete dollarsToCents in internal/adapters/clients/dhprice/batch_adapter.go and centsToDollars in internal/adapters/httpserver/handlers/pricing_api.go; replace their call sites with mathutil.ToCentsInt and mathutil.ToDollars respectively (note ToDollars takes int64, so pricing_api.go call sites need an int64 cast or a small int-accepting wrapper if that conversion is undesirable).
...
**Verifier correction:** The title's claimed consequence ('causing silent disambiguation misses') is not supported: PSA, BGS, CGC, and SGC grading scales never produce a grade with two or more fractional digits (grades are whole or single-decimal, e.g. 9, 9.5) -- so no real-world grader title can ever trigger the divergence the finding demonstrates with the synthetic input 'PSA 9.55'. The finding's own exclusions section already concedes this ('production PSA-sourced CSV data are effectively always single-decimal... flagged as a latent inconsistency rather than an active production bug'), so the self-reported severity is close to right, but the title overstates it as an active data-quality defect rather than a purely theoretical one. The real, defensible finding is narrower: three independently-maintained copies of the same regex fragment are a maintenance liability (the next person who needs a grade format change must remember to touch three files), not a currently-triggering bug.
...
### ARCH-005 — POINTER (dead-code lens): AppError.WithHTTPStatus appears to have no non-test caller, and its sibling HTTPStatus() has exactly one, in a retry policy

*Verdict:* `confirmed` · *Lens:* `architecture` · *Confidence:* `suspected`

**Why not ticketed:** Both evidence commands reproduce and the narrow factual claim is exactly right. Widening the search to include test files, `git grep -nE '\bWithHTTPStatus\b' -- 'internal/**/*.go' 'cmd/**/*.go'` shows every call site is in internal/domain/errors/errors_test.go (lines 161, 174, 266, 290, 297, 308); the only non-test hits are the declaration at errors.go:165 and two doc comments. HTTPStatus() has exactly the one non-test reader at internal/platform/resilience/retry.go:142. I answered the runtime checks the finding's exclusions declare it did not run: AppError is never embedded (the only non-test uses outside its own package are `var appErr *errors.AppError` type-assertion targets at campaigns_finance.go:266 and retry.go:91,135), no interface anywhere declares a WithHTTPStatus method, and there is no reflection or string-keyed route to the setter. Confidence 'suspected' and category 'dead-code' are correctly assigned, and the routing is correct — this belongs to the dead-code lens. I am marking it not ticketable on the LENS-BRIEF section 7 test rather than on its truth: its first acceptance criterion, 'Dead-code lens returns a verdict on WithHTTPStatus with all nine runtime checks answered', is an instruction to another audit lens, not something a fixer can prove; the second is conditional on a removal decision the finding deliberately does not make. A 'suspected' pointer is by construction recorded and not ticketed.

**What the finding got wrong:** Nothing wrong, but it stops one step short of a fact that strengthens it and that the dead-code lens will need. AppError.HTTPStatusCode is assigned in exactly one place in all non-test code — inside WithHTTPStatus itself (errors.go:167 and :169) — and the alternative population path HTTPStatus() consults, Context["http_status"] (errors.go:102), is set only in errors_test.go. Since WithHTTPStatus has no non-test caller, neither source is ever populated in production, so HTTPStatus() can only return 0 at runtime and the retry-policy branch at retry.go:142-143 that reads it is inert. That makes the reader dead too, not just the setter, which enlarges the removal candidate beyond what the finding scopes. I am recording this rather than inflating the finding on it, since ARCH-005 does not claim it.

### DCT-018 — The remaining three untested packages are low-risk and don't warrant a coverage ticket

*Verdict:* `confirmed` · *Lens:* `docs-config-tests` · *Confidence:* `suspected`

**Why not ticketed:** internal/domain/observability/ confirmed as doc.go+logger.go+noop.go with noop.go's 5 methods being one-line empty bodies. internal/domain/storage/ confirmed as doc.go only, no declarations; Cache interface confirmed to live at internal/platform/cache/cache.go:19. internal/testutil/config.go confirmed to contain only GetTestToken. However, `wc -l internal/testutil/config.go` actually returns 31, not the 25 quoted in the finding's evidence output — the file has a full doc comment above the function. The substantive conclusion (a single trivial helper function, low-risk, no test-coverage ticket warranted) still holds independently verified; only the specific line-count citation is wrong.

**What the finding got wrong:** The wc -l output quoted for internal/testutil/config.go ("25") does not reproduce; the actual line count is 31. This is a citation error, not a conclusion error — the file is still a single 6-line function with a long doc comment, and the finding's own recommendation is 'no ticket required,' so the miscount does not send anyone anywhere.

### DCT-021 — Prose referencing nonexistent symbols (function and interface names) is a recurring class, and nothing in the repo's tooling can ever catch it

*Verdict:* `confirmed_lower_severity` · *Lens:* `docs-config-tests` · *Confidence:* `strong`
...
**Why not ticketed:** Re-ran all three evidence commands verbatim: analytics_types.go:108-122 confirms TargetSellPrice/MinimumAcceptPrice sit among Cents-suffixed siblings without the suffix; service_sell_sheet.go confirms both are populated from cent-denominated sources (computeTargetPrice/ConservativeCents); sellSheetHelpers.tsx:40-42 confirms the frontend treats targetSellPrice as cents with no /100 conversion. The naming defect and cross-boundary propagation into the TS mirror are real and reproduce exactly as described.

**What the finding got wrong:** Nothing factually wrong, but it is filed as a suspected-tier pointer to the naming-and-boundaries lens rather than a frontend-health ticket candidate. Per LENS-BRIEF.md section 5, 'suspected' findings are recorded, not ticketed, and the finding's own exclusions block explicitly assigns ownership elsewhere. It should not be actioned or ticketed under frontend-health; ownership and any ticket decision belong to naming-and-boundaries.
...
*Claimed confidence:* `strong` · *Lens:* `docs-config-tests`

**How it was refuted:** The finding's subject.identity and claim both cite 'CLAUDE.md:145' as where the '`make check` — runs lint + architecture import check + file size check' text lives. Reading CLAUDE.md:145 directly shows a completely unrelated line: '- Service mocks for each sub-package: `MockArbitrageService`, `MockPortfolioService`, etc.' The actual `make check` description sentence is at CLAUDE.md:167 (confirmed via `grep -n "make check" CLAUDE.md`). The Makefile side of the evidence (`grep -n -A5 '^check:' Makefile`) does reproduce correctly — check: lint / check-imports.sh / check-file-size.sh / check-playwright-version.sh, at Makefile:146 as cited — confirming the Makefile does run a 4th, undocumented script. But per VERIFIER-BRIEF, a finding citing a line that does not contain the quoted text is refuted even when the underlying complaint is valid, because a fixer following the citation lands on the wrong sentence in CLAUDE.md.

**What it got wrong:** Cites CLAUDE.md:145 for the 'make check' description; the real location is CLAUDE.md:167. Line 145 is an unrelated bullet about mock patterns. The underlying complaint (Makefile's check target runs check-playwright-version.sh, which CLAUDE.md's description omits) is independently true, but the finding as written would send a fixer to the wrong line.

### DEADGO-001 — The entire platform/cache package is unwired: Config.Cache.Path, the -cache flag, its validation/mkdir, and cache.New have no live caller

*Claimed confidence:* `mechanical` · *Lens:* `dead-code-go`

**How it was refuted:** Every evidence command re-runs and produces the stated output, so I re-derived the claim independently and it splits in two. (A) The Go package IS externally unreachable, and I established this by a stronger route than the finding used: `git grep -n 'slabledger/internal/platform/cache' -- '*.go'` returns exactly ONE line tree-wide, internal/integration/pricing_test.go:12, and that file carries `//go:build integration` (line 4). One importer, integration-gated, settles external reachability for the whole package at once rather than symbol-by-symbol. No dot-import, no alias, no second importer; cmd/slabledger and cmd/psa-harvest both import nothing from it. (B) The rest of the cluster is LIVE, and the finding is affirmatively wrong about it. Dockerfile.harvest:39 is `ENTRYPOINT ["/app/psa-harvest", "--cache", "/tmp/psa-harvest-cache.json"]` — the deployed harvest container passes the flag on every start. The consumption chain is closed and entirely in-repo: cmd/psa-harvest/main.go:39 hands os.Args[1:] to parseBaselineFlag, which by construction passes `-cache` through untouched (cmd/psa-harvest/baseline_test.go:51,53 assert exactly that passthrough, in a default-build test); main.go:49 calls config.Load(args); loader.go:239 builds the FlagSet with `flag.ContinueOnError` and loader.go:262 returns fs.Parse's error; Load propagates it (loader.go:21-24); run() wraps it and main.go:44 calls log.Fatalf. Delete the `-cache` registration at loader.go:253 and that container dies at startup with `flag provided but not defined: -cache`. The mkdir is live for the same reason: Dockerfile.harvest:34-38 documents that EnsureDirectories runs in production and that the default `data/cache.json` would mkdir under the read-only /app WORKDIR and fail — the flag exists specifically to steer that mkdir to a writable path. Four of the five members named in the finding's own title (Config.Cache.Path, the -cache flag, its validation, EnsureDirectories' mkdir) are therefore live production surface.

**What it got wrong:** The root cause is the declared searched_universe: 'git-tracked *.go files'. A CLI flag's consumers are not Go source — they are Dockerfiles, compose files, and entrypoints — so the search was structurally incapable of seeing the one live consumer, and no amount of Go-side grepping would have surfaced it. Three consequences. (1) The title, subject identity, and severity assert a five-member dead cluster when only one member is dead; the finding navigated LENS-BRIEF trap 1 correctly on the Go side and then missed a live caller sitting outside the file glob. (2) proposed_fix instructs removal of 'the -cache CLI flag registration in loader.go' and 'EnsureDirectories'. Executed as written at mechanical tier — the tier that authorizes unqualified deletion — that breaks the deployed psa-harvest container. (3) acceptance_criteria #2 ('grep for ... \'-cache\' flag across the repo returns no remaining references') is unachievable and actively misleading: it would drive a fixer to strip Dockerfile.harvest:39 and the cmd/psa-harvest/baseline_test.go:51,53 fixtures, neither of which appears in blast_radius. A fixer working from the acceptance criteria alone cannot prove this fix correct, so it is not ticketable as written. Minor, separate: proposed_fix enumerates the package as 'cache.go, cache_test.go, cache_bench_test.go' and omits file_store.go and lru.go (blast_radius's `internal/platform/cache/*` does cover them).

### NB-007 — Campaign-targeting predicates are filed under the generic name `matching.go` inside `inventory`, while being the shared vocabulary of four packages plus a command

*Claimed confidence:* `suspected` · *Lens:* `naming-and-boundaries`

**How it was refuted:** The evidence reproduces — but it does not establish the defect. The claim is that the filename `matching.go` is a misnomer for campaign-targeting predicates. Reading the declarations it cites: ParseRange, MatchInput, PurchaseMatchesCampaign, LanguageAxisMatches, SubjectAxisMatches, SpecDenied, MatchResult, FindMatchingCampaign. Six of the eight exported symbols contain 'Match', and two of the three types are MatchInput and MatchResult. A file named matching.go containing the matching predicates is descriptive, not misleading. None of the three anchors applies: the name does not describe something the code no longer does, it does not collide with a different concept elsewhere (`git ls-files 'internal/domain/inventory/matching*'` returns only matching.go and matching_test.go), and it is not contradicted by its own package. The third evidence item — the comment block at portfolio.go:319-326 saying a nearby check deliberately does NOT delegate to SubjectAxisMatches — is offered as proof the concept 'has already begun to fork under the generic name', but the comment reads as a deliberate design decision documented against future 'simplification', which is the opposite of drift caused by a filename. The finding also concedes the case itself: confidence `suspected`, exclusions state it is 'a rename/relocate judgment with no mechanical seam behind it', and it says the relocation cannot be actioned before the NB-004 step-3 decision. What remains is a preference for `campaign_targeting.go` over `matching.go`. Refuted under the default-to-refuted rule; a fixer could execute the rename but could not demonstrate anything was wrong before it.

**What it got wrong:** It treats a generic-but-accurate filename as a defect without evidence of anyone being misled. Its own severity of `medium` is also unsupportable for a two-file intra-package rename that the finding says cannot be actioned yet. The one substantive observation buried in the exclusions — ParseRange is a generic string helper that likely belongs in internal/platform/ — is explicitly 'not separately assessed' and so carries no evidence either.

### NB-010 — POINTER (dead-code-go lens): DetectCoverageGaps, CoverageGap, ExtractCharacter, ClassifyEra and ClassifyPriceTier all show zero external references and are unambiguous in the map

*Claimed confidence:* `suspected` · *Lens:* `naming-and-boundaries`

**How it was refuted:** I verified the controller's refutation independently before recording it, and the controller is right on the facts. `git grep -nE '\b(DetectCoverageGaps|ExtractCharacter|ClassifyEra|ClassifyPriceTier)\b' -- 'internal/domain/inventory/*.go'` shows all four called from inside ComputePortfolioInsights: ExtractCharacter at portfolio.go:207 and :234, ClassifyEra at :224, ClassifyPriceTier at :229, DetectCoverageGaps at :246. ComputePortfolioInsights (portfolio.go:200) has three external call sites, all in the sibling package: internal/domain/portfolio/service.go:213, service.go:303, snapshot.go:227. CoverageGap is declared at portfolio_types.go:24, surfaced as `CoverageGaps []CoverageGap \`json:"coverageGaps"\`` at portfolio_types.go:48, and consumed by the frontend at web/src/types/campaigns/portfolio.ts:29 and :59. All five symbols are live; the cluster is reachable and serializes to the client. Not ticketable, and no removal should ever be attempted from this record. The map data the finding cites also reproduces exactly (all five refs=0 amb=false test_only=false) — the numbers are correct, the deadness inference from them is not.

**What it got wrong:** Nothing that the finding itself asserted — and the controller's paraphrase of it in ADJUDICATIONS.md overstates the claim, which the controller should know. ADJUDICATIONS.md renders the claim as 'five symbols are dead'. The finding never says that. Its title claims only that the five 'show zero external references and are unambiguous in the map' (true, and reproduced), its confidence is `suspected`, and its exclusions state outright: 'This lens did NOT run the nine runtime-reachability checks on these symbols; CoverageGap in particular carries JSON tags and could be an unreferenced-in-Go API contract (mechanism 4).' That is the exact mechanism that makes them live, named by the lens itself, before the controller found it. The verdict of refuted is still correct — the thing this pointer points at, a dead-code candidate cluster, is not real, and PREAMBLE rule 5 means a pointer that cannot survive is not worth routing — but the record should reflect that the lens hedged correctly rather than that it claimed deadness. This matters for Task 14: the lens's pointer discipline here was sound behavior to preserve, not a failure to correct.

See `docs/audit/ADJUDICATIONS.md` for the controller's full reasoning on DEADGO-001 and NB-010, including the production-breaking fix that DEADGO-001 would have shipped.
