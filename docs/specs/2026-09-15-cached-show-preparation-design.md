...
- Unresolved identities are reported as unavailable until existing identity enrichment supplies the required fields. That handoff must persist a verified grader-specific enrichment result on the purchase. A serial-only cached mapping is not itself verified identity: reuse requires the current purchase's existing verified profile to match the cached profile. Missing or conflicting cache provenance must use the existing grader-specific BuildCollectionCard enrichment. Failure must not promote the cache: missing identity stays unresolved, and an existing verified identity stays unchanged. Cover new enrichment and anchored cache reuse explicitly, including shared serials across graders. The worker does not guess a match or depend on CL collection membership.
...
- `internal/adapters/scheduler/cardladder_resolve.go`: the serial-only cached-mapping path can return before grader-specific enrichment. Cache presence alone is not provenance; tests must cover missing purchase identity and shared serials across graders.
